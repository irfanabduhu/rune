...
Turn a Maktabat al-Shamela book into a trilingual study edition: fully-voweled
(tashkeel) Arabic paragraphs, each followed by a lucid English translation and a
lucid Bengali translation (plus the editor's glosses), and a single self-contained
HTML reader.
...
3. **Translate** — for each chapter, fetch its pages (`fetch_page.py`), vowel the Arabic, translate into lucid English and Bengali, and write logical-paragraph units (no unit numbers; the print-page tag is the only marker). A whole book is fanned out to parallel agents per page range, assembled in order.
4. **Render** — `build_html.py` turns the book folder into one offline HTML reader (light/dark, RTL naskh typography, an All / Arabic / English / Bengali view switch, contents drawer with a page index).

Voweling is best-effort classical tashkeel; both translations are faithful and read as natural prose in their language.
